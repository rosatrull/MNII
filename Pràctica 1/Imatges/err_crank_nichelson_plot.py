...
ax.plot(X * L0, np.array(err_r1), label=f"$\\gamma = {solucions[0][1]}$")
ax.plot(X * L0, np.array(err_r2), label=f"$\\gamma = {solucions[1][1]}$")
ax.hlines(50, 0, 2, color='black', linestyles='dashed', alpha=0.7)

# Shaded region between z = 0.75 cm and z = 1.25 cm
ax.fill_betweenx([np.min([err_r1, err_r2]), np.max([err_r1, err_r2])+0.2], 0.75, 1.25, color='lightcoral', alpha=0.5, edgecolor='none')
...
ax.set_xlim(0, 2)
ax.set_ylim(ax.set_ylim(np.min([err_r1, err_r2]), np.max([err_r1, err_r2])+0.2))
